...
export function CampaignControls({ campaignId, status }: { campaignId: string; status: string }) {
  ...
  async function runNow() {
    ...
    if (res.ok) {
      const r = res.result
      toast.success(
        `Tick xong: ${r.draftsQueued} draft mới, ${r.followupsQueued} follow-up, ${r.rescheduledWindow} đẩy window, ${r.yieldedToEngagement} nhường AE, ${r.suppressed} suppressed, ${r.nurtured} nurture, ${r.draftFailures} lỗi`,
        { duration: 8000 },
      )
      router.refresh()
    } else {
